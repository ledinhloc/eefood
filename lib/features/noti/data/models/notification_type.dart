...
class NotificationType {
  ...
  static final Map<String, Map<String, dynamic>> typeMap = {
    'COMMENT': {'icon': Icons.mode_comment_rounded, 'color': Colors.blue},
    'REACTION': {'icon': Icons.favorite_rounded, 'color': Colors.pinkAccent},
    'FOLLOW': {'icon': Icons.person_add_rounded, 'color': Colors.green},
    'SYSTEM': {
      'icon': Icons.notifications_active_rounded,
      'color': Colors.orange,
    },
    'LIVESTREAM': {'icon': Icons.live_tv_rounded, 'color': Colors.redAccent},
    'SAVE_RECIPE': {'icon': Icons.bookmark_rounded, 'color': Colors.teal},
    'SHARE_RECIPE': {'icon': Icons.share_rounded, 'color': Colors.indigo},
    'WELCOME': {'icon': Icons.waving_hand_rounded, 'color': Colors.amber},
    'RECIPE_OF_THE_DAY': {
  ...
  static String? detectReactionType(String? body) {
    ...
    if (lower.contains('love') || lower.contains('tim') || lower.contains('❤️'))
      return 'LOVE';
    if (lower.contains('ngạc nhiên') || lower.contains('wow')) return 'WOW';
